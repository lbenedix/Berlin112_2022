...
max_id = None
for _ in range(35):
    result = twitter.search(q=f'(#{hashtag}) (from:{account})',
                            result_type='recent',
                            count=100,
                            max_id=max_id)
    ...
    for tweet in tweets:
        ...
        if hashtag.lower() in hash_tags:  # and not tweet['in_reply_to_screen_name']:
            t = OrderedDict()
            t['bezirk'] = ''
            t['text'] = tweet['text']
            ...
            t['id'] = tweet['id']
            t['url'] = 'https://twitter.com/{}/status/{}'.format(account, tweet['id'])
...
json.dump(
    all_tweets,
    open('Berlin112_2022.json', 'w', encoding='utf8'),
    indent=2,
    ensure_ascii=False,
    sort_keys=False
)
